...
//            Block, BlockGrass, BlockTallGrass, BlockLeaves, 
//            IBlockAccess, BiomeGenBase, ColorizerFoliage, Material, 
//            MathHelper, EntityFX, Vec3D, BlockStem
...
public class CustomColorizer
{
    ...
    {
        java.io.InputStream inputstream = renderengine.getTexturePack().field_6534_a.func_6481_a(s);
        if(inputstream == null)
        {
            return;
        }
    ...
    private static void readCustomPalettes(Properties properties, RenderEngine renderengine)
    {
        ...
        for(int j = 0; j < as.length; j++)
        {
            ...
            for(int k = 0; k < as1.length; k++)
            {
                ...
                if(l != Block.field_337_v.field_376_bc && l != Block.field_9257_X.field_376_bc && l != Block.field_384_L.field_376_bc && l != Block.field_35278_bv.field_376_bc)
                {
                    blockPalettes[l] = j;
                }
            }
    ...
    private static int[] getCustomColors(String s, RenderEngine renderengine, int i)
    {
        ...
        int ai[] = renderengine.func_28149_a(s);
        if(ai == null)
        {
            return null;
        }
    ...
    public static int getColorMultiplier(Block block, IBlockAccess iblockaccess, int i, int j, int k)
    {
        ...
        byte byte0 = 0;
        int j1 = 0;
        if(block == Block.field_337_v || block == Block.field_9257_X)
        {
            byte0 = 1;
        ...
        if(block == Block.field_384_L)
        {
            byte0 = 2;
            flag1 = Config.isSmoothBiomes();
            j1 = iblockaccess.func_602_e(i, j, k);
            if((j1 & 3) == 1)
            {
        ...
        if(ai1 != ai && iblockaccess.func_48454_a(i, k) == BiomeGenBase.field_4255_b)
        {
            ai = ai1;
        }
        if(ai != null)
        ...
        {
            return block.func_207_d(iblockaccess, i, j, k);
        }
    }

    private static int getSmoothColorMultiplier(Block block, IBlockAccess iblockaccess, int i, int j, int k, int ai[], int ai1[], int l, 
            int i1)
    {
        int j1 = 0;
        int k1 = 0;
        ...
        for(int i2 = i - 1; i2 <= i + 1; i2++)
        {
            ...
            {
                int ai2[] = ai;
                if(ai1 != ai2 && iblockaccess.func_48454_a(i2, k2) == BiomeGenBase.field_4255_b)
                {
                    ai2 = ai1;
                }
                int j3 = 0;
                if(ai2 == null)
                {
                    ...
                    {
                    case 1: // '\001'
                        j3 = iblockaccess.func_48454_a(i2, k2).func_48415_j();
                        break;

                    case 2: // '\002'
                        if((i1 & 3) == 1)
                        {
                            j3 = ColorizerFoliage.func_21175_a();
                        } else
                        if((i1 & 3) == 2)
                        {
                            j3 = ColorizerFoliage.func_21174_b();
                        } else
                        {
                            j3 = iblockaccess.func_48454_a(i2, k2).func_48412_k();
                        }
                        break;

                    default:
                        j3 = block.func_207_d(iblockaccess, i2, j, k2);
                        break;
                    }
                } else
                {
    ...
    {
        BiomeGenBase biomegenbase = iblockaccess.func_48454_a(i, k);
        double d = MathHelper.func_48442_a(biomegenbase.func_48411_i(), 0.0F, 1.0F);
        double d1 = MathHelper.func_48442_a(biomegenbase.func_48414_h(), 0.0F, 1.0F);
        d1 *= d;
    ...
    public static void updatePortalFX(EntityFX entityfx)
    {
        ...
        } else
        {
            ...
            entityfx.field_663_i = f;
            entityfx.field_662_j = f1;
            entityfx.field_661_k = f2;
            return;
        }
    }
    ...
    public static void updateReddustFX(EntityFX entityfx, IBlockAccess iblockaccess, double d, double d1, double d2)
    {
        ...
        int i = iblockaccess.func_602_e((int)d, (int)d1, (int)d2);
    ...
    public static void updateWaterFX(EntityFX entityfx, IBlockAccess iblockaccess)
    {
        ...
        int i = (int)entityfx.field_611_ak;
        int j = (int)entityfx.field_610_al;
        int k = (int)entityfx.field_609_am;
        int l = getFluidColor(Block.field_399_C, iblockaccess, i, j, k);
        int i1 = l >> 16 & 0xff;
        int j1 = l >> 8 & 0xff;
        int k1 = l & 0xff;
    ...
    public static int getLilypadColor()
    {
        if(lilyPadColor < 0)
        {
            return Block.field_40200_bA.func_35274_i();
        } else
        {
            return lilyPadColor;
        }
    ...
    public static Vec3D getSkyColor(Vec3D vec3d, IBlockAccess iblockaccess, double d, double d1, double d2)
    {
        ...
        } else
        {
            ...
            float f1 = (float)k / 255F;
            float f2 = (float)l / 255F;
            float f3 = (float)vec3d.field_1776_a / 0.5F;
            float f4 = (float)vec3d.field_1775_b / 0.66275F;
            float f5 = (float)vec3d.field_1779_c;
            f *= f3;
            f1 *= f4;
            f2 *= f5;
            return Vec3D.func_1248_b(f, f1, f2);
        }
    }
    ...
    public static Vec3D getFogColor(Vec3D vec3d, IBlockAccess iblockaccess, double d, double d1, double d2)
    {
        ...
        } else
        {
            ...
            float f1 = (float)k / 255F;
            float f2 = (float)l / 255F;
            float f3 = (float)vec3d.field_1776_a / 0.753F;
            float f4 = (float)vec3d.field_1775_b / 0.8471F;
            float f5 = (float)vec3d.field_1779_c;
            f *= f3;
            f1 *= f4;
            f2 *= f5;
    ...
    public static int getSmoothColor(int ai[], IBlockAccess iblockaccess, double d, double d1, double d2, 
            int i, int j)
    {
        if(ai == null)
        {
    ...
    public static int getStemColorMultiplier(BlockStem blockstem, IBlockAccess iblockaccess, int i, int j, int k)
    {
        if(stemColors == null)
        {
            return blockstem.func_207_d(iblockaccess, i, j, k);
        }
